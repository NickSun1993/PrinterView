    ...
    [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            else
            {
                ...
                [AlertCenter ShowAlertWithController:self andTitle:@"温馨提示！" andMessage:title andCancelTitle:@"好的" andCancelActionBlock:nil andOtherTitle:nil andActionBlock:nil];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count > 0)
    {
        ...
        NSString *name = [NSString stringWithFormat:@"%@",metadataObject.type];
        
        NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:name,@"Type",result,@"Content", nil];
